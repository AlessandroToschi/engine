...
void LayerTree::Flatten(const SkRect& bounds,
                        RenderSurfaceProvider* render_surface) {
  auto* canvas = render_surface->get_canvas();
  ...
  assert(render_surface->get_canvas() != nullptr);
  assert(render_surface->get_context() != nullptr);
  assert(render_surface->get_color_space() != nullptr);
  assert(render_surface->get_raster_cache() != nullptr);

  PrerollContext preroll_context{
      render_surface->get_raster_cache(),
      render_surface->get_context(),  // gr_context  (used for the raster cache)
      nullptr,                        // external view embedder
      unused_stack,                   // mutator stack
      render_surface->get_color_space(),  // SkColorSpace* dst_color_space
      kGiantRect,                         // SkRect cull_rect
      false,                              // layer reads from surface
      unused_stopwatch,                   // frame time (dont care)
      unused_stopwatch,                   // engine time (dont care)
  ...
  Layer::PaintContext paint_context = {
      static_cast<SkCanvas*>(&internal_nodes_canvas),
      canvas,  // canvas
      render_surface->get_context(),
      nullptr,
      unused_stopwatch,                    // frame time (dont care)
      unused_stopwatch,                    // engine time (dont care)
      unused_texture_registry,             // texture registry (not supported)
      render_surface->get_raster_cache(),  // raster cache
      false,                               // checkerboard offscreen layers
      device_pixel_ratio_                  // ratio between logical and physical
  };
